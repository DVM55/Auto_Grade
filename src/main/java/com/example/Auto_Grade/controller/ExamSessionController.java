...
public class ExamSessionController {
    ...
    @PreAuthorize("hasRole('TEACHER')")
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> updateExamSession(
    ...
    @PreAuthorize("hasRole('TEACHER')")
    @GetMapping("/{examId}")
    public ResponseEntity<PagingResponse<ExamSessionResponse>> getExamSessions(
            @PathVariable Long examId,
        ...
        return ResponseEntity.ok(
                examSessionService.getExamSessionByExamId(examId, sessionName, page, size)
        );
    }
}